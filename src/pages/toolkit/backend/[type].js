...
//     const response = await fetch(`https://devitools.vercel.app/api/paths`)
...
export const getServerSideProps = async ({ params }) => {
    ...
    // const response = await fetch(`http://localhost:3000/api/tools/backend/${type}`)
    const response = await fetch(`https://devitools.vercel.app/api/tools/backend/${type}`)
    const data = await response.json()
    // console.log(data)
